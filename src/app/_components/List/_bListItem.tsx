...
const BlindBoxItem: React.FC<BlindBoxItemProps> = ({
  ...
  const itemContent = (
    <div
      className={`${isSelected ? 'bg-primary006' : ''} relative border-white009
        ${
          viewMode === 'list'
            ? 'flex items-center px-4 border-t-[1px]'
            : 'border rounded'
        } `}
    ...
    >
      <div
        className={`${
          viewMode === 'list' ? 'w-12 h-12 mr-4' : 'h-[120px]'
        } relative`}
      >
        <Image
          alt={blindBox.id!}
          src={`${
            blindBox.boxData.length
              ? '/svg/blindbox-animation-6.svg'
              : '/svg/blindbox-animation-1.svg'
          }`}
          className="rounded"
          layout="fill"
          objectFit="cover"
      ...
      <div className="w-[115px] ml-4 h-[80px] flex-grow flex flex-col items-start justify-center">
        ...
        <p className="text-white003 font-SourceSanPro text-labelmb text-white001 max-w-[300px] truncate">
          {blindBox.boxData.length
            ? `${blindBox.boxData.length} ${
                blindBox.boxData.length === 1 ? 'Gift' : 'Gifts'
              }`
            : 'Empty Box'}
        </p>
      </div>
      {isSelected && (
        <div
          className={`absolute ${
            viewMode === 'list' ? 'right-4' : 'top-2 right-2'
  ...
  return interactionType && interactionType > 1 ? (
    ...
    <Link href={`/blindBox/${blindBox.id}`}>{itemContent}</Link>
  );
};
