...
import useQueryDobByAddress, {
  QueryDobByAdderssItem,
} from '@/hooks/useQueryDobByAddress';
import { filterItemsByClusterId } from '@/utils/supportedDobList';
...
const UserHome: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'Gift' | 'Blind Box'>(
    decodeURIComponent(searchParams.get('type') || '') === 'Blind Box'
      ? 'Blind Box'
      : 'Gift',
  );
  ...
  const { data: spores, loading: isSporesLoading } = useQueryDobByAddress();
  const storeSporesList = useSelector(
    (state: RootState) => state.spores.spores,
  );
  const [sporesList, setSporesList] = useState<QuerySpore[] | []>([]);
  const [blindBoxList, setBlindBoxList] = useState<[]>([]);
  const [supportedSpores, setSupportedSpores] =
    useState<QueryDobByAdderssItem[]>();
  ...
  useEffect(() => {
    if (spores) {
      const list = filterItemsByClusterId(spores);
      setSupportedSpores(list);
    }
  }, [spores]);
  ...
  const renderContent = () => {
    ...
    const currentList = activeTab === 'Gift' ? supportedSpores : blindBoxList;

    if (currentList && supportedSpores && supportedSpores.length > 0) {
      return (
        <GiftList
          onNewGiftClick={handleOpenModal}
          list={supportedSpores}
          type={activeTab}
          blindboxList={blindBoxList}
        />
      );
    } else {
      return (
        <div>
          <div className="relative my-8 min-h-[170px]">
            <Image
              ...
              src={`${
                activeTab === 'Gift'
                  ? '/svg/ps-gift-empty.svg'
                  : '/svg/ps-blindbox-empty.svg'
              }`}
              layout="fill"
              objectFit="contain"
            />
          ...
          <p className="text-center my-8 text-white001 text-hd2mb font-Montserrat">
            {`${
              activeTab === 'Gift'
                ? 'Start Creating Your First On-Chain Gift!'
                : 'Start Creating Your First Blind Box!'
            }`}
          </p>
        </div>
      );
  ...
  return (
    <div className="container mx-auto px-4 pb-12">
      <div className="flex rounded-md bg-primary011 mt-8">
        <button
          className={`flex-1 py-2 m-1 text-white font-SourceSanPro ${
            activeTab === 'Gift'
              ? 'bg-primary010 text-labelbdmb'
        ...
        <button
          className={`flex-1 py-1 m-1 text-white font-SourceSanPro ${
            activeTab === 'Blind Box'
              ? 'bg-primary010 text-labelbdmb'
              : 'text-labelmb text-white005'
          } rounded-md `}
      ...
      <Button
        ...
        label={'Create ' + activeTab}
      />
      <Tip className="mt-6" type={activeTab} />
      {/* {isModalOpen && (
        <CreateModal
          title={`Create New ${activeTab}`}
          onClose={handleCloseModal}
        >
          {activeTab === 'Gift' ? (
            <CreateGift onClose={handleCloseModal} />
          ) : (
            <CreateBlindBox
              onClose={handleCloseModal}
