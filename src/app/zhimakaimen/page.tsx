...
const Zhimakaimen: React.FC = () => {
  ...
  const [pageStatus, setPageStatus] = useState<
    'process' | 'successful' | 'fail'
  >('process');
  const [giftMessage, setGiftMessage] = useState<string>('');
  const [sporeInfo, setSporeInfo] = useState<HashkeyGift>();
  const tweetText =
    "I just unlocked an exclusive New Year gift with #PhilosopherStone at the event co-hosted by #BuidlerDAO! 🎉 Join the game and claim yours. Don't miss out! 🚀";
      ...
      {pageStatus === 'process' && (
        <>
          ...
          <div className="w-full flex flex-col items-center justify-center mt-8">
            <p className="text-white001 font-Montserrat text-hd3mb">
              Unlock Your New Year Gift
            </p>
            <p className=" text-white003 font-SourceSanPro text-labelmb mt-4">
              Enter the key you&#39;ve discovered from the riddles below
            </p>
          </div>
          <div className="w-full flex items-center flex-col justify-center mt-8">
            ...
            />
            {errMsg && (
              <p className="font-SourceSanPro text-light-error-function text-labelmb mt-4">
                {errMsg}
              </p>
            )}
          </div>
      ...
      {pageStatus === 'fail' && (
        <>
          <div className="w-full flex justify-center items-center flex-col mt-8">
            <Image
              alt={'already-claim'}
              src={'/svg/already-claim.svg'}
              width={160}
              height={170}
            />
            <p className="text-white001 font-Montserrat text-hd3mb mt-8">
              {failBody.head}
            </p>
            ...
            <Button
              type="solid"
              className="max-w-[343px] mx-auto flex justify-center items-center mt-8"
              label="Back to My Gifts"
              href={'/'}
            />
          </div>
        </>
        ...
        <>
          <div className="relative mt-10 w-full flex flex-col items-center py-8 px-4 bg-primary008 rounded-md font-SourceSanPro text-white001">
            ...
            <div className="w-full flex justify-center mt-4">
              <img
                src={`/api/media/${sporeInfo?.sporeId}`}
                className="px-4 w-[220px] h-[170px]"
                alt="Gift"
              />
            </div>
          ...
          <p className="text-center text-white001 font-Montserrat text-hd3mb mt-6">
            Congratulation!
          </p>
          <p className="text-center text-white003 mt-4 font-SourceSanPro text-labelmb">
            The Gift is yours to enjoy. Please note, it might take some time to
            appear in your Gifts.
          </p>
          <Button
            type="solid"
            className="max-w-[343px] mx-auto flex justify-center items-center mt-8"
          ...
          <div className="w-full text-center mt-4">
            <Link
              href={'/'}
              className="font-SourceSanPro text-linkColor text-center text-body1mb"
            >
              Back to My Gifts
            </Link>
          </div>
