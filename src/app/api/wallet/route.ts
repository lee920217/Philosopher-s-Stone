...
function handleError(error: string, errno: number = 400) {
  return NextResponse.json(
    { error: 'Collect Gift Failed, Try agian later', errno },
    { status: errno },
  );
}
...
async function signAndSendTransaction(
  ...
  await kv.lpush(`${receiverAccounts}-received`, {
    id: sporeId,
    date: new Date(),
  });
...
export async function signAndSendTransactionApi(
  sporeId: string,
  receiverAccounts: string,
) {
  const privateKey = process.env.PRIVATE_WALLET_KEY;
