...
const create = async (k: string, boxName: string, ids = []) => {
  ...
  return NextResponse.json(
    { data: [], message: 'successful', errno: 200 },
    { status: 200 },
  );
};
...
const getInBlindBoxList = async (k: string) => {
  ...
  return NextResponse.json(
    { data: rlt, message: 'successful', errno: 200 },
    { status: 200 },
  );
};

const getBlindBoxByName = async (
  k: string,
  boxName: string,
...
const add = async (k: string, boxName: string, giftIds: string[]) => {
  ...
  return NextResponse.json(
    { data: currBlindBoxes[boxIndex].boxData, errno: 200 },
    { status: 200 },
  );
};
...
const remove = async (k: string, boxName: string, giftIds: string[]) => {
  ...
  currBlindBoxes[boxIndex].boxData = currBlindBoxes[boxIndex].boxData.filter(
...
const clear = async (k: string, giftIds: string[]) => {
  ...
  currBlindBoxes.forEach(blindBox => {
    blindBox.boxData = blindBox.boxData.filter(
      gift => !giftIds.includes(gift.id),
    );
  });
...
const send = async (k: string, boxName: string) => {
  ...
  if (!box || box.boxData.length === 0) {
    return handleError(
      'No Blind Box found with the given name or Blind Box is empty.',
    );
  }
  ...
  return NextResponse.json(
    { giftId: giftObject.id, errno: 200 },
    { status: 200 },
  );
};
...
export async function POST(req: NextRequest, res: NextApiResponse) {
  ...
  switch (body.action) {
    ...
    case 'getList':
      return NextResponse.json(
        { data: await getList(`${body.key}-blindbox`), errno: 200 },
        { status: 200 },
      );
    case 'add':
