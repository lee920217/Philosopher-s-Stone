...
const checkAndGetHashKey = async (
  k: string,
  ...
  try {
    ...
    return NextResponse.json(
      { data: { ...rlt, tradeTx: txHash }, errno: 200 },
      { status: 200 },
    );
  } catch (error) {
...
const saveHashkey = async (k: string, record: HashkeyObj) => {
  ...
  await kv.lpush(`${record.senderWalletAddress}-hashKey`, {
    [k]: { ...record, date },
  });
  ...
  switch (body.action) {
    ...
    case 'checkAndGetHashKey':
      return await withErrorHandling(
        ...
        req.cookies.get('__gpi')?.value.split('UID=')[1] || '',
      );
    default:
