...
const Finished: React.FC = () => {
  ...
  const tweetText = encodeURIComponent(
    `🌠 I just created a box of gifts with #PhilosopherStone! Don’t miss out on this one-of-a-kind surprise. Claim it here: ${window.location.protocol}//${window.location.host}/collect/${key} 🎁 Feel free to melt it into #CKB if it's not your cup of tea. ✨ \n\n #NFT #Gift #SporeProtocol #Web3`,
  );

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(
        `${window.location.protocol}//${window.location.host}/collect/${key}`,
      );
      ...
      {type === 'URL' ? (
        ...
        <>
          ...
          <div className="text-white001 text-center font-SourceSanPro">
            Depending on network traffic, it might take a little time to
            complete. You can always check the status in your{' '}
            <Link href={'/history'} className="text-linkColor">
              Gift History
            </Link>
            , or visit the{' '}
            <Link
            ...
            . Thank you for spreading joy with Philosopher&apos;s Stone! 🌟
          </div>
          <Button
            type="solid"
            className="flex justify-center items-center"
