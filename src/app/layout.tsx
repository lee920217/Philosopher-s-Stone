...
// Customized styling for the Snackbar notifications
...
// Configuration for ConnectProvider
const connectConfig = {
  autoConnect: true,
  ...
  return (
    <html lang="en" className="min-h-full min-w-full">
      <head>
        ...
        <Script
          async
          src="https://www.googletagmanager.com/gtag/js?id=G-23YX4KSBQE"
        ></Script>
        <Script id="gtag-init">
          {`
      ...
      <body className="bg-desktop-bg bg-no-repeat min-h-full min-w-full m-0">
        ...
        <ccc.Provider>
          <TrpcProvider>
            <ConnectProvider value={connectConfig}>
              <GiftReceiveModalProvider>
                ...
                >
                  <Provider store={store}>
                    {width >= 768 && (
                      <div className="sticky top-0 left-0 w-full mb-8 z-50">
                        <DesktopHeader />
                      </div>
                    )}
                    <div className="container relative flex flex-col min-h-screen mx-auto sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
                      {width < 768 && <Header />}
                      <div
                        className={`flex-1 flex flex-col w-full overscroll-y-contain bg-gradient-conic ${
                          width < 768 ? 'rounded-none' : 'rounded-3xl'
                        }`}
                        style={{
